...
interface VehicleApi {
  ...
  userName?: string // láº¥y tá»« VehicleResponse backend
}
...
export default function VehiclePage() {
  ...
  const loadVehicles = async () => {
    try {
      const res = await apiClient.get<VehicleApi[]>("/api/admin/vehicles")
      ...
      const mapped: Vehicle[] = data.map((v) => ({
        ...
        userName: v.userName ?? undefined, // dÃ¹ng Ä‘Ãºng userName tá»« backend
      }))
  ...
  const handleUserNameChange = async (userName: string) => {
    ...
    try {
      ...
      if (user) {
        setForm((prev) => ({
          ...prev,
          userId: user.userID, // set UserID tá»« API
        }))
      }
    } catch (err) {
    ...
    try {
      if (editing) {
        const payload = {
          ...
          userID: editing.userId, // giá»¯ user cÅ© khi edit
        }
        await apiClient.put(`/api/admin/vehicles/${editing.id}`, payload)
      } else {
        const payload = {
          vin: form.vin,
          vehicleModel: form.vehicleModel,
          batteryType: form.batteryType,
          registerInformation: form.registerInformation,
          userID: form.userId, // Ä‘Ã£ Ä‘Æ°á»£c auto-fill tá»« userName
        }
        await apiClient.post("/api/admin/vehicles", payload)
      }
