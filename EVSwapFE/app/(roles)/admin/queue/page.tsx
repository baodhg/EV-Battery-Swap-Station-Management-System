...
export default function AdminQueuePage() {
  // ====== SOFT GUARD (khÃ´ng redirect) ======
  const { user, isLoggedIn, isLoading, authChecked } = useAuth();
  const isAdmin = (user?.role ?? "").toString().toLowerCase() === "admin";

  if (!authChecked || isLoading) {
    return <p className="p-6">Äang kiá»ƒm tra quyá»n truy cáº­pâ€¦</p>;
  }
  if (!isLoggedIn) {
    return (
      <Result
        status="warning"
        title="401"
        subTitle="Báº¡n cáº§n Ä‘Äƒng nháº­p Ä‘á»ƒ xem trang nÃ y."
        extra={
          <a href="/signin">
            <Button>ÄÄƒng nháº­p</Button>
          </a>
        }
      />
    );
  ...
  if (!isAdmin) {
    return (
      <Result
        status="403"
        title="403"
        subTitle="Báº¡n khÃ´ng cÃ³ quyá»n truy cáº­p trang Admin."
        extra={
          <a href="/">
            <Button variant="outline">Vá» trang chá»§</Button>
          </a>
        }
      />
    );
  ...
  const fetchToday = async () => {
    try {
      ...
      // });
      // if (!res.ok) throw new Error("KhÃ´ng táº£i Ä‘Æ°á»£c danh sÃ¡ch");
      // const data = (await res.json()) as Booking[];
      // setBookings(data);
      setBookings(demo); // fallback demo
    } catch {
      setBookings(demo); // fallback náº¿u lá»—i
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="p-8">
        ...
        <div>
          ...
          <Card className="bg-white">
            {loading ? (
              <div className="p-6 text-sm text-gray-500">Äang táº£iâ€¦</div>
            ) : bookings.length === 0 ? (
              <div className="p-6 text-sm text-gray-500">KhÃ´ng cÃ³ booking hÃ´m nay</div>
            ) : (
